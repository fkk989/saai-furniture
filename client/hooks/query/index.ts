...
export const useUpdateQuery = () => {
  ...
  const mutation = useMutation({
    ...
    mutationFn: async ({ id, body }: { id: string; body: object }) => {
      toast.loading("updating project", { id: "update-query" });
      const data = (
        await axios.put(`${baseUrl}/query/update`, {
          id,
          ...body,
        })
      ).data;
    ...
    onSuccess: () => {
      toast.success("successfull", { id: "update-query" });
      queryClient.invalidateQueries({ queryKey: ["get-query"] });
    },
    onError: () => {
      toast.error("error", { id: "update-query" });
    },
  });
